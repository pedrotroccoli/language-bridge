# Squashed baseline: collapses the pre-release, dev-only migration history into a
# single genesis migration. On a fresh database `db:prepare`/`db:setup` loads
# db/schema.rb directly (db:schema:load) and never runs this; it exists so
# `db:migrate` from empty still bootstraps the full schema.
