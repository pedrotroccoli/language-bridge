---
title: CDN setup
description: Put the public delivery endpoint behind a CDN.
---

The public delivery endpoint is built to sit behind a CDN. The CDN absorbs read
traffic; the origin only serves cache misses and revalidations.

```
i18n client ──▶ CDN (edge cache) ──▶ origin  GET /cdn/:slug/:locale/:ns(.json)
                     │                      │
...
No CDN. Requests hit the origin directly; the same `Cache-Control`/`ETag` headers
